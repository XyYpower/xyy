...
开始工作前：
1. **先读 `docs/CONTEXT.md`** 了解项目全貌和最新进度（全量档案）
2. **再读 `docs/HANDOFF.md`** 了解上次会话干到哪、下次该干什么（增量便签）
3. `git status` 确认工作区是否干净，理解未提交改动

## 会话交接（强制）

不同工具（Claude Code / Codex / ZCode）的会话记忆互不共享，**唯一同步通道是项目文件**。因此：

- **会话开始时**：读 `docs/HANDOFF.md`，从「待办」接着干
- **会话结束时**：更新 `docs/HANDOFF.md`——把本次内容写进「最近一次会话」，更新「待办」清单，旧的记录往下移（保留最近 ~5 次）
- 如有架构/API/Schema 变更，**同时**更新 `docs/CONTEXT.md`

分工：`CONTEXT.md` = 全量档案（稳定）；`HANDOFF.md` = 当前便签（高频改动）。
...
- 当前任务：`docs/V2.2-task.md`（V2.2 AI 导入 + 学习路径）
